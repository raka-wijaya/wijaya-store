...
function Footer() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(
        "Terima kasih telah berlangganan! Langganan Anda segera dikonfirmasi."
      );
      setEmail("");
    } catch (error) {
      console.error("Gagal mengirim email:", error);
      setMessage(
        "Terjadi kesalahan saat berlangganan. Silakan coba lagi nanti."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <footer
      id="About"
      className="container p-5 bg-[#F0F0F0] text-[#333] font-sans relative top-20 z-0"
    >
      ...
      <div className="bg-black text-white w-full max-w-[1240px] h-auto mx-auto rounded-3xl py-12 px-6 md:px-10 lg:px-16 flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="flex flex-col items-center sm:flex-row gap-4 w-full md:w-auto">
          <form
            ...
            className="flex flex-col items-center space-y-4 w-full sm:w-[280px]"
          >
        ...
        <div>
          ...
          <div className="flex space-x-4">
            ...
            <a
              ...
              className="w-10 h-10 flex items-center justify-center bg-white rounded-full hover:bg-gray-400 transition-colors"
            >
              <Instagram size={20} />
            </a>
            <a
